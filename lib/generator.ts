 ...
 * 生成元音辅音组合的字符串
 * @param length 字符串长度
 * @param startWithConsonant 是否以辅音开头(默认true)
 * @returns 元音辅音交替组合的字符串
 */
function generateVowelConsonantString(length: number, startWithConsonant: boolean = true): string {
  let result = '';
  let useConsonant = startWithConsonant;
  
  for (let i = 0; i < length; i++) {
    if (useConsonant) {
      result += CONSONANTS[secureRandom(0, CONSONANTS.length - 1)];
    } else {
      result += VOWELS[secureRandom(0, VOWELS.length - 1)];
    }
...
/**
 * 生成随机长度(6-8位)的元音辅音组合字符串
 */
function generateRandomVowelConsonantString(): string {
  const length = secureRandom(6, 8);
  const startWithConsonant = Math.random() > 0.5;
  return generateVowelConsonantString(length, startWithConsonant);
}
...
function generateEnglishName(): string {
  const length = secureRandom(3, 5);
  const name = generateVowelConsonantString(length, true);
  // 首字母大写
...
export function generateName(countryCode: string) {
  ...
  // 对于英语国家(US, CA, GB, AU, SG等),使用元音辅音组合生成英文名
  const englishCountries = ['US', 'CA', 'GB', 'AU', 'SG', 'NZ', 'IE'];
  
  if (englishCountries.includes(countryCode)) {
    const firstName = generateEnglishName();
    const lastName = generateEnglishName();
...
/**
 * 生成密码：6-8位元音辅音组合的英文字母
 */
export function generatePassword(): string {
  return generateRandomVowelConsonantString();
}
...
export function generateEmail(firstName: string, lastName: string, customDomain?: string) {
  ...
  const username = generateRandomVowelConsonantString();
