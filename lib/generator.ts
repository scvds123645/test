 ...
 */
function generateRandomLetters(minLength: number = 6, maxLength: number = 9): string {
  const length = secureRandom(minLength, maxLength);
  let result = '';
  for (let i = 0; i < length; i++) {
...
export function generatePassword(): string {
  return generateRandomLetters(6, 9);
}
...
export function generateEmail(firstName: string, lastName: string, customDomain?: string) {
  ...
  const username = generateRandomLetters(6, 9);
